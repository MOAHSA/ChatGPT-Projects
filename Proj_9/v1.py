...
class Player:
    ...
    def collect_item(self, item):
        ...
        print(f"Player State after collecting item: Health: {self.health}, PlayerSpeed: {self.temp_speed_boost}, Bullet Speed: {self.bullet_speed}, Bullet Count: {self.temp_bullet_count}, Attack Damage: {self.attack_damage}, Untouchable: {self.untouchable}")
...
class Enemy:
    ...
    def check_for_collisions(self, enemies):
        ...
        for enemy in enemies:
            if enemy != self and self.position.distance_to(enemy.position) < 30:  # Collision radius
                return True
        return False
...
class Item:
    ...
    def display_effect_text(self, screen):
        if self.effect_text_surface is None:
            effect_text = self.effect.replace('_', ' ').upper()
            color = COLORS['good_item_color'] if self.item_type == 'good' else COLORS['bad_item_color']
            ...
            self.effect_text_timer = pygame.time.get_ticks()  # Start the timer

        # Draw the effect text
