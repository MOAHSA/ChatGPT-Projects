...
class NmapScannerApp:
    ...
    def create_expert_commands_tab(self, frame):
        """Create tab for common expert Nmap commands"""
        ...
        expert_commands = {
            "Host Discovery": [
                ("Basic Network Scan", "nmap -sn {target}/24"),
                ("ARP Discovery", "nmap -PR -sn {target}/24"),
                ("No Ping Scan", "nmap -Pn {target}"),
                ("TCP SYN Ping", "nmap -PS21,22,23,25,80,443,3306,3389 {target}"),
            ],
            "Port Scanning": [
                ("Full TCP Port", "nmap -p- {target}"),
                ("Fast Port Scan", "nmap -F {target}"),
                ("Service Version", "nmap -sV -sS -T4 {target}"),
                ("All TCP Scans", "nmap -sS -sT -sA -sW -sM {target}"),
            ],
            "Stealth Scanning": [
                ("Stealth SYN Scan", "nmap -sS -T2 {target}"),
                ("Decoy Scan", "nmap -D RND:5 {target}"),
                ("Idle Scan", "nmap -sI zombie_host {target}"),
                ("FTP Bounce", "nmap -b ftp_relay {target}"),
            ],
            "Advanced Detection": [
                ("OS and Services", "nmap -A -T4 {target}"),
                ("Aggressive Scan", "nmap -A -T4 -v {target}"),
                ("NSE Vuln Scan", "nmap --script vuln {target}"),
                ("All Default NSE", "nmap -sC {target}"),
            ],
            "Firewall Evasion": [
                ("Fragment Packets", "nmap -f {target}"),
                ("Change MTU", "nmap --mtu 24 {target}"),
                ("Spoof MAC", "nmap --spoof-mac Dell/Apple/0 {target}"),
                ("Source Port", "nmap --source-port 53 {target}"),
            ],
            "Web Application": [
                ("HTTP Services", "nmap -p80,443 -sV --script=http-* {target}"),
                ("WordPress Scan", "nmap -sV --script http-wordpress-* {target}"),
                ("SQL Injection", "nmap --script http-sql-injection {target}"),
                ("XSS Vulnerabilities", "nmap --script http-xssed {target}"),
            ]
        }
        
        for category, commands in expert_commands.items():
            ...
            for name, cmd in commands:
                # Create command button with tooltip
                btn = ttk.Button(
                    category_frame,
                    text=name,
    ...
    def load_expert_command(self, command):
        """Load expert command into command preview"""
        # Get current target
        target = self.target_entry.get() or "{target}"
    ...
    def run_scan(self):
        """Enhanced scan execution with better error handling and feedback"""
        ...
        try:
            ...
            self.notebook.select(self.notebook.index('end') - 3)  # Results tab
            
            # Clear and prepare output
            self.clear_output()
            self.output_text.insert(tk.END, f"Initializing scan...\n\nCommand: {command}\n\n")
            ...
            self.progress_bar.start(10)  # Smoother animation
            self.update_scan_status("Scan in progress...")
            
            # Add to history if history list exists
            ...
            self.update_scan_status("Scan failed to start")
    ...
    def process_output(self):
        """Optimized output processing with batching"""
        if self.is_updating:
            return
        
        self.is_updating = True
        try:
            # Process output in larger batches
            batch_size = 50
            lines = []
            ...
            for _ in range(batch_size):
                try:
                    ...
                    if line is None:  # Scan completed
                        self.stop_button.config(state=tk.DISABLED)
                        self.progress_bar.stop()
                        lines.append("\nScan completed.")
                        break
            ...
            if lines:
                # Single batch update with all lines
            ...
            self.root.after(200, self.process_output)  # Increased interval to 200ms

    def stop_scan(self):
        """Stop the current scan"""
        if self.scan_running:
            self.scan_running = False  # Signal thread to stop
...
For security guidelines: https://nmap.org/book/legal-issues.html"""
    ...
    def update_scan_status(self, status):
        """Update the scan status label with animation"""
        ...
        if self.scan_running:
            ...
            self.status_label.config(text=f"{char} {status}")
