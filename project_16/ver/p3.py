...
class RecipeManager(tk.Tk):
    ...
    def create_recipe_card(self, recipe_name, recipe_info):
        ...
        # View button - using tk.Button for consistent styling
    ...
    def manage_recipe(self, old_name=None):
        ...
        # Get recipe name
        new_name = simpledialog.askstring(title, "Enter recipe name:", initialvalue=initial_value)
        if not new_name:
            return
        ...
        recipe_dialog = RecipeDetailsDialog(self, f"{title} Details", initial_data=current_details)
        recipe_details = recipe_dialog.result
        
        if not recipe_details:  # User canceled
            return
